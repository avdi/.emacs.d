...
(defun make-rubytapas-frame ()
  ...
  (let* ((frame-alist '((fullscreen . maximized)
    ...
    (select-frame frame)
    (set-frame-font "Source Code Pro:pixelsize=30:foundry=adobe:weight=normal:slant=normal:width=normal:spacing=100:scalable=true" nil)
...
(defun rubytapas-scratch-buffer ()
  ...
  (whitespace-mode -1)
  (menu-bar-mode -1)
  (if (featurep 'flycheck)      (flycheck-mode -1))
  (if (featurep 'company)       (company-mode -1))
  (if (featurep 'guru-mode)     (guru-mode -1))
  (if (featurep 'flyspell)      (flyspell-mode -1))
  (if (featurep 'enh-ruby-mode) (setq enh-ruby-check-syntax nil))
  ;; (set (make-local-variable 'xmpfilter-command-name)
  ;;      "ruby -S xmpfilter --no-warnings --dev --fork --detect-rbtest")
  (if (featurep 'fci-mode)      (fci-mode -1))
